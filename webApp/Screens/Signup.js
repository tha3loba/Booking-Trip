...
const Login = ({navigation}) => {
    ...
    const sendCred = async () => {
        try {
            let response = await axios.post('http://10.0.2.2:5000/api/users/register-user', { ...Data });
            console.log(Data)
            AsyncStorage.setItem('token',response.data.token)       
            navigation.replace("Browse")
        } catch (error) {
                console.log(error.response.data)
            Alert.alert("Erreur", 'Invalid Signup credentials', [
                {
           ...
           <View style={styles.container}>
               ...
               <Animatable.View style={styles.footer} animation="fadeInUpBig">
                    ...
                    <TouchableOpacity
                        ...
                        >
                            <Text style={styles.textSignupButton}>Login</Text>
                        </TouchableOpacity>
